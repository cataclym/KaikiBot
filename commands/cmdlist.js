...
module.exports = {
	...
	async execute(message) {
		...
		const CommandsCollection = await message.client.commands; // Gets all command objects
		const CmdsList = await CommandsCollection.map(t => t.cmdCategory ? t : null).filter(a => !!a).sort(); // Filter out the nulls
		...
		for (const [i, item] of CmdsList.entries())
		{
