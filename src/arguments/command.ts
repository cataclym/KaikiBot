...
export class CommandArgument extends Argument<KaikiCommand> {
    public run(parameter: string, context: Argument.Context<KaikiCommand>): Argument.AwaitableResult<KaikiCommand> {
        ...
        const aliasResult = <KaikiCommand>container.stores.get("commands")
            .aliases
            .get(parameter);

        const partialResult = cmds
            .sort((firstValue, secondValue) => firstValue.name.length - secondValue.name.length)
            .find(k => {
                const name = k.name.toLowerCase();
                const param = parameter.toLowerCase();

                return param
                    .startsWith(name.slice(0, Math.max(param.length - 1, 1)));
            });
