...
export default class Roles {
    ...
    static async handleStickyRoles(member: GuildMember) {
        ...
        if (result.success) {
            container.logger.info(`stickyRoles | Re-added roles to ${colorette.blueBright(member.user.username)} [${colorette.blueBright(member.id)}]`);
        }

        else {
            container.logger.warn(`stickyRoles | Bot cannot add roles due to role-hierarchy!! ${colorette.blueBright(member.guild.name)} [${colorette.blueBright(member.guild.id)}]`);
        }
    }

    static async restoreUserRoles(member: GuildMember): Promise<{ success: boolean, roles: Role[] } | false> {
        const { guild } = member;
