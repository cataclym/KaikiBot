...
import { Awaitable, ChatInputCommandInteraction, EmbedBuilder, InteractionResponse, Message } from "discord.js";
    ...
    private createEmbedMessage(interaction: ChatInputCommandInteraction | Message, initialMsg: Message | InteractionResponse) {
        return {
            embeds: [
                new EmbedBuilder()
                    .addFields([
                        { name: "WebSocket ping", value: Math.abs(interaction.client.ws.ping) + " ms", inline: true },
                        { name: "Client ping", value: (initialMsg.createdTimestamp - interaction.createdTimestamp) + " ms", inline: true },
                    ])
                    .withOkColor(interaction.inGuild() ? interaction.guild! : undefined),
            ],
                content: null,
        }
    }
