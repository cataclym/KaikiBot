...
export default class MyRoleCommand extends Subcommand {
    ...
    public async nameRun(message: Message<true>, args: Args) {
            ...
            return message.channel.send({
                embeds: [
                    await KaikiEmbeds.embedFail(
                        message,
                        "This role is higher than me, I cannot edit this role!"
                    ),
                ],
            });
        }
        ...
        return message.channel.send({
            embeds: [
                new EmbedBuilder()
                    .setDescription(
                        `You have changed \`${oldName}\`'s name to \`${name}\`!`
                    )
    ...
    public async colorRun(message: Message<true>, args: Args) {
        ...
        return message.channel.send({
            embeds: [
                new EmbedBuilder()
                    .setDescription(
                        `You have changed \`${myRole.name}\`'s color from \`${oldHex}\` to \`${hex}\`!`
                    )
                    .setColor(hex),
            ],
        });
    ...
    public async iconRun(message: Message<true>, args: Args) {
            ...
            return message.channel.send({
                embeds: [
                    await KaikiEmbeds.errorMessage(
                        ...
                        "This server does not have enough boosts for role-icons!"
                    ),
                ],
            });
        }
        ...
        if (args.finished) {
            ...
            const icon = await args.rest(
                KaikiArgumentsTypes.emoteImageArgument
            );
                ...
                message.channel.send({
                    embeds: [
                        (
                            ...
                            name: "Message",
                            value: await KaikiUtil.codeblock(err, "xl"),
                        }),
                    ],
                })
            );
            ...
            return message.channel.send({
                embeds: [
                    new EmbedBuilder()
                        .setDescription(
                            `You have set \`${myRole.name}\`'s icon!`
                        )
                        .withOkColor(message),
                ],
            });
